<template>
  <div class="q-pa-none">
    <q-btn dense flat icon="info" :disable="disable">
      <q-menu>
        <q-table flat bordered dense row_key="key" :rows="rows" :columns="columns" separator="vertical" hide-header hide-bottom :loading="loading">
          <template v-slot:top>
...
export default {
  ...
  watch: {
    versions: function (newVal, oldVal) { // watch it
      ...
      //this.rows[4].value = newVal.git_hash;
    }
  },
  data() {
    return {
      ...
      rows: [
        ...
        { key: "Client API version:", value: "TODO" },
      ],
      columns: [
